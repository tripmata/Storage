    ...
    $(document).ready(function(){
        ...
        alertSound = getElement("alertSound");
    ...
    function initializePOS()
    {
        ...
        cover.innerHTML =
            ...
            "<div class='ui inline active loader'></div> Initializing Pastry POS" +
            "</h1><br/><br/>" +
    ...
    function calculateCouponDiscount()
    {
        ...
        for(let i = 0; i < cc.length; i++)
        {
            ...
            {
                if(coupons[j].Pastry.includes(c))
                {
    ...
    function loadImages()
    {
        if(pl < imageList.length)
        {
            let image = new Image();
            image.onload = function () {
                if(typeof imageLoadProgress == "function")
                {
                    imageLoadProgress(Math.round(30 + ((pl + 1) * (70 / imageList.length))));
                }
                pl++;
                loadImages();
            }
            image.src = "files/" + imageList[pl].Images[0];
        }
        else
        {
                        ...
                        loadPOSReceipt(function (data, status) {
                            if(status === "success")
                            {
                                ...
                                loadItemList(function (data, status) {
                                    ...
                                    imageLoadProgress = function(percent){
                                        $(".load-progress-con").html(percent);
                                        if(o.onProgress != null)
                                        {
                                            if(typeof o.onProgress == "function")
                                            {
    ...
    function initRoutine()
    {
        ...
        try
        {
            que = JSON.parse(window.localStorage.getItem("bakery_data_que"));
        }
        catch (e)
        {
            try
            {
                que = JSON.parse(window.localStorage.getItem('bakery_que_store'));
            }
            catch (e)
            {
    ...
    function runRoutine(o)
    {
        if(!queWork)
        {
            ...
            let que = JSON.parse(window.localStorage.getItem("bakery_data_que"));
    ...
    function removeFromQue(o)
    {
        if(!queWork)
        {
            ...
            if(que != null)
            {
                ...
                if(que.length > 0)
                {
                    window.localStorage.setItem("bakery_data_que", JSON.stringify(que));
                ...
                {
                    window.localStorage.removeItem("bakery_data_que");
    ...
    let loop = false;
    function newOnlineOrder(count)
    {
        //count show the number of recent orders

        $("#alert-num").html(notifications.length.toString());
        $("#alert-num").show();

        if(!$("#order-bell").hasClass("slash"))
        {
            loop = true;
            $("#order-bell").transition('set looping').transition('tada');

            alertSound.addEventListener('ended', function(){
                if(loop){alertSound.play();}
            });
            alertSound.play();
        }
    }
    ...
    function stopAlert()
    {
        loop = false;
        $("#alert-num").hide();
        $("#order-bell").transition('remove looping');
        $("#alert-num").html("0");
    }

    function openFoodOrder(m)
    {
        stopAlert();
        ...
        {
            if((foodOrder[i] != null) && (!foodOrder[i].Fullfilled))
            {
                content +=
                    "<div id='"+foodOrder[i].Id+"-con' class='pad-1 w3-row hoverable' " +
                    "style='border-bottom: 1px solid lightgray; cursor: pointer;'  " +
                    "onclick=\"openSingleOrder('"+foodOrder[i].Id+"','"+$("#order-modal-id").val()+"')\">" +
                    "<div class='w3-col l9 m9 s9' style='line-height: 180%;'>" +
                    "<span>"+(foodOrder[i].Room != null ? "room "+foodOrder[i].Room[0].Number : "Unknown room")+"</span>" +
                    "<br/><span>"+foodOrder[i].Pastrypixel.length+" item(s)</span>" +
                    "</div> " +
                    "<div class='w3-col l3 m3 s3 align-r' style='line-height: 180%;'>" +
                    "<span>"+(foodOrder[i].Paid ? "<span class='green'>Paid</span>" :
                    "<span class='red'>Unpaid</span>")+"</span>" +
                    "<br/><span>"+$("#currency-symbol").val()+
                    numFormat(Number((Number(foodOrder[i].Total) + Number(foodOrder[i].Taxes)) - Number(foodOrder[i].Discount)).toFixed(2))+"</span>" +
                    "</div> " +
                    "</div>";
            }
        }
        ...
        if(content === "")
        {
            content =
                "<div class='align-c pad-3'>" +
                "<h3 class='ui icon header'>" +
                "<i class='birthday icon' style='color: silver;'></i>" +
                "</h3>" +
                "<h4 class='sleak' style='font-weight: bold; color: dimgray;'>" +
                "There are no pastry orders yet" +
                "</h4>" +
                "</div>";
        }

        if(getElement("food-order-con") != null)
        {
            $("#food-order-con").html(content);
        }
        else
        {
            loadPageModal({size:"s", onLoaded:function(m){
                ...
                $("#modal_"+m.modal+"-inner").html(
                    ...
                    "<i class='blue birthday cake icon'></i> Pastry Order" +
                    "</h2>" +
                    "</div>" +
                    "</div>" +
                    "<hr style='margin: 0px; padding: 0px;'/>" +
                    "<input id='order-modal-id' type='hidden' value='"+m.modal+"'/> <br/>" +
                    "<div id='food-order-con'>"+content+"</div>");
            }});
        }
    }

    function openSingleOrder(e)
    {
        $("#food-order-con").html("");
        ...
        if(order !== null)
        {
            ...
            for(let i = 0; i < order.Pastrypixel.length; i++)
            {
                list +=
                    "<div class='w3-row'>" +
                        "<div class='w3-col l4 m4 s4 pad-1'><img src='files/"+
                    (order.Pastrypixel[i].Pastry.Images[0] != null ? order.Pastrypixel[i].Pastry.Images[0] : "")+"' " +
                        "style='width: 50px; border-radius: 6px;'/></div>" +
                        "<div class='w3-col l5 m5 s5 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        order.Pastrypixel[i].Pastry.Name+"</h6></div>" +
                        "<div class='w3-col l3 m3 s3 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        numFormat(Number(order.Pastrypixel[i].Quantity))+"</h6></div>" +
                    "</div>";
            }
            content +=
                "<div class='w3-row pad-1'>" +
                "<div class='w3-col l6 m6 s6'> " +
                "<button class='ui icon blue button' onclick='openFoodOrder()'>" +
                "<i class='arrow left icon'></i></button>" +
                "</div>" +
                "<div class='w3-col l6 m6 s6 align-r'> " +
                "<button id='cancel-order-btn' class='ui icon basic red button' onclick=\"confirmCancelOrder('"+order.Id+"')\">" +
                "<i class='trash red icon'></i> Cancel order</button>" +
                "</div>" +
                "</div>" +
                ...
                "<tr>" +
                "<td>Payment</td>" +
                "<td>"+(order.Paid ? "<span class='green sleak' style='font-weight: bold;'>Paid</span>" :
                "<span class='red sleak' style='font-weight: bold;'>Unpaid</span>")+"</td>" +
                "</tr>" +
                "<tr>" +
                "<td>Room</td>" +
                "<td>"+order.Room[0].Number+"</td>" +
                "</tr>" +
                "<tr>" +
                "<td class='pad-1'>Customer</td>" +
                ...
                "<table class='ui very basic no-line table'>" +
                "<tr>" +
                "<td class='pad-1'>Subtotal</td>" +
                "<td>"+$("#currency-symbol").val()+numFormat(Number(order.Total).toFixed(2))+"</td>" +
                "</tr>" +
                "<tr>" +
                "<td class='pad-1'>Tax</td>" +
                "<td>"+$("#currency-symbol").val()+numFormat(Number(order.Taxes).toFixed(2))+"</td>" +
                "</tr>" +
                "<tr>" +
                "<td class='pad-1'>Discount</td>" +
                "<td>"+$("#currency-symbol").val()+numFormat(Number(order.Discount).toFixed(2))+"</td>" +
                "</tr>" +
                "<tr>" +
                "<td class='pad-1'>Total</td>" +
                "<td>"+$("#currency-symbol").val()+
                numFormat(((Number(order.Total) + Number(order.Taxes)) - (order.Discount)).toFixed(2))+"</td>" +
                "</tr>" +
                "</table>" +
                "</div>" +
                "<hr style='margin: 0px;'/>" +
                "<div class='pad-1' style='margin-bottom: 20px;'>" +

                (!order.Paid ?

                    "<div class='ui fluid labeled input' style='margin-top: 10px;'>" +
                    "<label class='ui label'>Amount "+$("#currency-symbol").val()+"</label>" +
                    "<input id='web-payment-amount' type='text' placeholder='Paid amount' value='"+
                    ((Number(order.Total) + Number(order.Taxes)) - (order.Discount)).toFixed(2)+"'/>" +
                    "</div>" +
                    "<div class='w3-row' style='margin-top: 10px; margin-bottom: 10px;'>" +

                        (settings.Cash ? "<div class='w3-col l4 m4 s4'>" +
                    "<label><input id='web_cash_pay' class='with-gap' name='order_pay_method' type='radio'/><span>Cash</span></label>" +
                    "</div>" : "") +
                        (settings.Pos ?
                    "<div class='w3-col l4 m4 s4'>" +
                    "<label><input id='web_pos_pay' class='with-gap' name='order_pay_method' type='radio'/><span>POS</span></label>" +
                    "</div>" : "") +
                        (settings.Others ?
                    "<div class='w3-col l4 m4 s4'>" +
                    "<label><input web_others_pay class='with-gap' name='order_pay_method' type='radio'/><span>Others</span></label>" +
                    "</div>" : "") +
                    "</div>"
                ...
                "<button class='ui blue sleak button' onclick=\"pushWebOrder('"+order.Id+"')\">Process & print receipt</button> " +
                "</div>" +
                "</div>";
        }
        else
        {
            content +=
                "<div class='align-c pad-3'>" +
                "<h3 class='ui icon header'>" +
                "<i class='utensils icon' style='color: silver;'></i>" +
                "</h3>" +
                "<h4 class='sleak' style='font-weight: bold; color: dimgray;'>" +
                "Invalid pastry order" +
                "</h4>" +
                "</div>";
        }
        ...
        {
            loadPageModal({
                size: "s", onLoaded: function (m) {

                    $("#modal_" + m.modal + "-inner").html(
                        "<div class='pad-2'>" +
                        "<div class=''>" +
                        "<h2 style='font-family: quicksandregular; color: dimgrayy'>" +
                        "<i class='blue birthday icon'></i> Pastry Order" +
                        "</h2>" +
                        "</div>" +
                        "</div>" +
                        "<hr style='margin: 0px;'/>" +
    ...
    {
        ConfirmModal("Are you sure you want to cancel the order?", function (choice, param) {
            if(choice)
            {
                cancelOrder(param);
            }
    ...
    {
        loadingButton({btn:"cancel-order-btn"});
        postJson("hms-pos/worker", function(data, status){
            loadingButton({btn:"cancel-order-btn", loading:false});
            if(status === "done")
            {
                let d = JSON.parse(data);
                ...
                if(d.Status === "success")
                {
                    for(let i = 0; i < foodOrder.length; i++)
                    {
                        if(foodOrder[i].Id === e)
                        {
                            foodOrder.splice(i, 1);
                        }
                    }

                    $("#cancel-order-btn").addClass("positive");
                    $("#cancel-order-btn").html("<i class='check icon'></i> Order canceled");
                    setTimeout(function(){
                        openFoodOrder();
                    },2000);
                }
                ...
                {
                    errorButton({btn:"cancel-order-btn", msg:d.Message});
                }
            }
            else
            {
                errorButton({btn:"cancel-order-btn", msg:"Connection error"});
            }
        },{job:"cancel order", item_type:$("#pos-type").val(), order:e});
    }

    function pushWebOrder(e)
    {
        let order = buildWebOrder(e);
    ...
    function buildWebOrder(e)
    {
        ...
        if(od === null)
        {
            ShowModal("Order seems to be damaged. Please reload page and try again");
        }
        else
        {
            let order = {};
            ...
            order.taxes = od.Taxes;
            order.discount = od.Discount;

            order.transId = generateId();
            order.time = (Date.now() / 1000);

            let cc = od.Pastrypixel;

            for(let i = 0; i < cc.length; i++)
            {
                order.items.push(cc[i].Pastry.Id+":"+cc[i].Quantity+":"+cc[i].Price+":"+cc[i].Tax);
                order.printItems.push({item:cc[i].Pastry.Name,qty:cc[i].Quantity,price:cc[i].Price});
            }
            order.entity = od.Customer.Id+":"+"customer";
            order.coupons = [];
            order.discounts = [];

            order.posuser = $("#pos-user").val();

            order.paidAmount = !od.Paid ? Number($("#web-payment-amount").val()) : 0;
            order.isWeborder = true;
            order.weborder = od.Id;
            order.method = od.Paid ? "web" : "";

            if((order.paidAmount > 0) && (!od.Paid))
            {
                if((getElement("web_cash_pay") != null) && (getElement("web_cash_pay").checked))
                {
                    order.method = "cash";
                }
                else if((getElement("web_pos_pay") != null) && (getElement("web_pos_pay").checked))
                {
                    order.method = "pos";
                }
                else if((getElement("web_others_pay") != null) && (getElement("web_others_pay").checked))
                {
                    order.method = "others";
                }
                else
                {
                    ShowModal("Select payment method to proceed");
                    return ;
                }
            }
            ...
            {
                ShowModal("There are no items in the order. Empty orders cannot be processed");
            }
        }
    }
    //--------------------------------------End of online order processing---------------------------------------
    ...
    function launchsaveOrder()
    {
        if(!pushed)
        {
            let savedList = JSON.parse(window.localStorage.getItem("bakery_sys_saved_order"));
    ...
    function saveOrder()
    {
        ...
        if(order.items.length > 0)
        {
            ...
            if((listingName === "menu") || (listingName === "bakery_sys_saved_order") || (listingName === ""))
            {
            ...
            else
            {
                ...
                window.localStorage.setItem("bakery_sys_saved_order", JSON.stringify(savedList));
    ...
    function openSavedOrder(e)
    {
        ...
        loadPageModal({size:"s", onLoaded:function(m){
            ...
            $("#modal_"+m.modal+"-inner").html(
                ...
                "<div class=''>" +
                "<h2 style='font-family: quicksandregular; color: dimgrayy'>" +
                "<i class='blue save icon'></i> Saved Order" +
                "</h2>" +
                "</div>" +
                "</div>" +
    ...
    function calculateAddedDiscount()
    {
        ...
        for(let i = 0; i < cc.length; i++)
        {
            ...
            {
                if(addedDiscount[j].Pastry.includes(c))
                {
    ...
    function fetchNotification(e)
    {
        let content = "";
        for(let i = 0; i < notifications.length; i++)
        {
            if((notifications[i] != null) && (!notifications[i].Fullfilled))
            {
                content +=
                    "<div id='"+notifications[i].Id+"-con' class='pad-1 w3-row hoverable' " +
                    "style='border-bottom: 1px solid lightgray; cursor: pointer;'  " +
                    "onclick=\"$('#noteDropdown').toggle(); openSingleOrder('"+notifications[i].Id+"')\">" +
                    "<div class='w3-col l9 m9 s9 align-l' style='line-height: 180%;'>" +
                    "<span>"+(notifications[i].Room != null ? "room "+notifications[i].Room[0].Number : "Unknown room")+"</span>" +
                    "<br/><span>"+notifications[i].Pastrypixel.length+" item(s)</span>" +
                    "</div> " +
                    "<div class='w3-col l3 m3 s3 align-r' style='line-height: 180%;'>" +
                    "<span>"+(notifications[i].Paid ? "<span class='green'>Paid</span>" :
                    "<span class='red'>Unpaid</span>")+"</span>" +
                    "<br/><span>"+$("#currency-symbol").val()+
                    numFormat(Number((Number(notifications[i].Total) + Number(notifications[i].Taxes)) - Number(notifications[i].Discount)).toFixed(2))+"</span>" +
                    "</div> " +
                    "</div>";
            }
        }
        ...
        {
            content =
                "<div class='align-c pad-1'>" +
                "<h5 class='ui icon header'>" +
                "<i class='bell icon' style='color: silver;'></i>" +
                "No new pastry orders yet" +
                "</h5>" +
                "</div>";
        }

        $("#noteDropdown").html(content);
    }
    ...
    function toggleAlert()
    {
        ...
        {
            alertSound = false;
            $("#alert-toggle-con").html("<i class='bell icon'></i> Enable order alert");
            stopAlert();
            $("#order-bell").css("color","silver");
            $("#order-bell").removeClass("red");
            $("#order-bell").addClass("slash outline");
        }
        else
        {
            alertSound = true;
            $("#alert-toggle-con").html("<i class='bell icon'></i> Silence order alert");
            $("#order-bell").css("color","");
            $("#order-bell").addClass("red");
            $("#order-bell").removeClass("slash outline");
        }
    }
    ...
    function showMenu()
    {
        ...
        cover.innerHTML =
            ...
            "<i class='birthday cake icon'></i> Pastry Menu</h3>" +
            "</div>" +

            "<!--<div class='w3-col l7 m6 s12'>" +
            "<div class='ui fluid search'>" +
            "<div class='ui icon fluid input'>" +
            "<input class='wix-textbox' type='text' placeholder='Search'/>" +
            ...
            "<div id='menu-body' class='w3-row pad-1'>" +
            "</div>" +
        ...
        for(let i = 0; i < menu.length; i++)
        {
            ...
            m.className = "w3-col l6 m6 s6";
            m.style.padding = "5px";
            m.innerHTML = "<div class='food-item' style='position: relative;' onclick=\"AddItem('"+menu[i].Id+"'); foodItemClicked(this)\">" +
                "<img src='files/"+menu[i].Images[0]+"' style='width: 100%;'/>" +
                "<div class='' style='position: absolute; width: 100%; bottom: 0px; background-color: rgba(0,0,0,0.5);'>" +
                "<h6 style='font-family: quicksandregular; color: white; text-align: center;'>" +
                "<span style='font-family: Arial;'>"+
                $("#currency-symbol").val()+"</span>"+numFormat(Number(menu[i].Price).toFixed(2))+"</h6>" +
                "</div> " +
                "<h4 class='status blue-back' style='font-family: quicksandregular; position: absolute;" +
                " top: 5px; left: 5px; margin: 0px;'>"+menu[i].Name+"</h4>" +
                "</div>";
    ...
    function AddItem(item)
    {
        ...
        for(let i = 0; i < menu.length; i++)
        {
            if((menu[i].Id === item) || (menu[i].Barcode === item))
            {
                e = menu[i];
            }
        }
        ...
        $("#"+e).transition('fade down', function () {
            ...
            if(document.getElementsByClassName("sale-item").length == 0)
            {
                $("#product-con").html("<tr>" +
                    ...
                    "<i class='shopping basket circular blue-text icon'></i> " +
                    "Pastry basket is empty</h3></td></tr>");
            }
        });
    ...
    function calculateDiscount()
    {
        ...
        for(let i = 0; i < cc.length; i++)
        {
            ...
            for(let j= 0; j < discounts.length; j++)
            {
                ...
                {
                    if(discounts[j].Pastry.includes(c))
                    {
    ...
    function addOrderToQue(order)
    {
        ...
        que.push(order);
        window.localStorage.setItem('bakery_que_store', JSON.stringify(que));
        window.localStorage.setItem('bakery_data_que', JSON.stringify(que));

        window.localStorage.removeItem('bakery_que_store');
