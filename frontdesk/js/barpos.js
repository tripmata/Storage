    ...
    function initializePOS()
    {
        ...
        cover.innerHTML =
            ...
            "<div class='ui inline active loader'></div> Initializing Bar POS" +
            "</h1><br/><br/>" +
    ...
    function initRoutine()
    {
        ...
        try
        {
            que = JSON.parse(window.localStorage.getItem("bar_data_que"));
        }
        catch (e)
        {
            try
            {
                que = JSON.parse(window.localStorage.getItem('bar_que_store'));
            }
            catch (e)
            {
    ...
    function runRoutine(o)
    {
        if(!queWork)
        {
            ...
            let que = JSON.parse(window.localStorage.getItem("bar_data_que"));
    ...
    function removeFromQue(o)
    {
        if(!queWork)
        {
            ...
            if(que != null)
            {
                ...
                if(que.length > 0)
                {
                    window.localStorage.setItem("bar_data_que", JSON.stringify(que));
                ...
                {
                    window.localStorage.removeItem("bar_data_que");
    ...
    function openFoodOrder(m)
    {
        ...
        for(let i = 0; i < foodOrder.length; i++)
        {
            if((foodOrder[i] != null) && (!foodOrder[i].Fullfilled))
            {
                content +=
                    ...
                    "<br/><span>"+foodOrder[i].Drinkpixel.length+" item(s)</span>" +
                    "</div> " +
        ...
        if(content === "")
        {
            content =
                "<div class='align-c pad-3'>" +
                "<h3 class='ui icon header'>" +
                "<i class='martini glass icon' style='color: silver;'></i>" +
                "</h3>" +
                "<h4 class='sleak' style='font-weight: bold; color: dimgray;'>" +
                "There are no drink orders yet" +
                "</h4>" +
                "</div>";
        }
        ...
        else
        {
            loadPageModal({size:"s", onLoaded:function(m){
                ...
                $("#modal_"+m.modal+"-inner").html(
                    ...
                    "<i class='blue martini glass icon'></i> Drink Order" +
                    "</h2>" +
                    "</div>" +
                    "</div>" +
    ...
    function openSingleOrder(e)
    {
        ...
        if(order !== null)
        {
            ...
            for(let i = 0; i < order.Drinkpixel.length; i++)
            {
                list +=
                    "<div class='w3-row'>" +
                        "<div class='w3-col l4 m4 s4 pad-1'><img src='files/"+order.Drinkpixel[i].Drink.Images[0]+"' " +
                        "style='width: 50px; border-radius: 6px;'/></div>" +
                        "<div class='w3-col l5 m5 s5 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        order.Drinkpixel[i].Drink.Name+"</h6></div>" +
                        "<div class='w3-col l3 m3 s3 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        numFormat(Number(order.Drinkpixel[i].Quantity))+"</h6></div>" +
                    "</div>";
            }
            content +=
            ...
            loadPageModal({
                size: "s", onLoaded: function (m) {
                    ...
                    $("#modal_" + m.modal + "-inner").html(
                        ...
                        "<div class=''>" +
                        "<h2 style='font-family: quicksandregular; color: dimgrayy'>" +
                        "<i class='blue martini icon'></i> Drink Order" +
                        "</h2>" +
                        "</div>" +
                        "</div>" +
                        "<hr style='margin: 0px;'/>" +
    ...
    function buildWebOrder(e)
    {
        ...
        else
        {
            ...
            let cc = od.Drinkpixel;

            for(let i = 0; i < cc.length; i++)
            {
                order.items.push(cc[i].Drink.Id+":"+cc[i].Quantity+":"+cc[i].Price+":"+cc[i].Tax);
                order.printItems.push({item:cc[i].Drink.Name,qty:cc[i].Quantity,price:cc[i].Price});
            }
    ...
    function launchsaveOrder()
    {
        if(!pushed)
        {
            let savedList = JSON.parse(window.localStorage.getItem("bar_sys_saved_order"));
    ...
    function saveOrder()
    {
        ...
        if(order.items.length > 0)
        {
            ...
            if((listingName === "menu") || (listingName === "bar_sys_saved_order") || (listingName === ""))
            {
            ...
            else
            {
                ...
                window.localStorage.setItem("bar_sys_saved_order", JSON.stringify(savedList));
    ...
    function calculateAddedDiscount()
    {
        ...
        for(let i = 0; i < cc.length; i++)
        {
            ...
            {
                if(addedDiscount[j].Drink.includes(c))
                {
        ...
        for(let i = 0; i < notifications.length; i++)
        {
            if((notifications[i] != null) && (!notifications[i].Fullfilled))
            {
                content +=
                    ...
                    "<br/><span>"+notifications[i].Drinkpixel.length+" item(s)</span>" +
                    "</div> " +
                    "<div class='w3-col l3 m3 s3 align-r' style='line-height: 180%;'>" +
    ...
    function showMenu()
    {
        ...
        cover.innerHTML =
            ...
            "<i class='martini glass icon'></i> Bar Menu</h3>" +
            "</div>" +
        ...
        $("#"+e).transition('fade down', function () {
            ...
            if(document.getElementsByClassName("sale-item").length == 0)
            {
                $("#product-con").html("<tr>" +
                    ...
                    "<i class='utensils circular blue-text icon'></i>" +
                    "Food Tray is empty</h3></td></tr>");
            }
        });
    ...
    function emptyTray()
    {
        $("#product-con").html("<tr>" +
            "<td colspan='5'>" +
            "<h3 class='sleak pad-2' style='color: silver;'>" +
            "<i class='martini glass circular blue-text icon'></i> " +
            "Drinks Tab is empty</h3></td></tr>");
    ...
    function calculateDiscount()
    {
        ...
        for(let i = 0; i < cc.length; i++)
        {
            ...
            for(let j= 0; j < discounts.length; j++)
            {
                ...
                {
                    if(discounts[j].Drink.includes(c))
                    {
    ...
    function addOrderToQue(order)
    {
        ...
        que.push(order);
        window.localStorage.setItem('bar_que_store', JSON.stringify(que));
        window.localStorage.setItem('bar_data_que', JSON.stringify(que));

        window.localStorage.removeItem('bar_que_store');
