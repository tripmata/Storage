    ...
    function initializePOS()
    {
        ...
        cover.innerHTML =
            ...
            "<div class='ui inline active loader'></div> Initializing pool POS" +
            "</h1><br/><br/>" +
    ...
    function initRoutine()
    {
        ...
        try
        {
            que = JSON.parse(window.localStorage.getItem("pool_data_que"));
        }
        catch (e)
        {
            try
            {
                que = JSON.parse(window.localStorage.getItem('pool_que_store'));
            }
            catch (e)
            {
    ...
    function runRoutine(o)
    {
        if(!queWork)
        {
            ...
            let que = JSON.parse(window.localStorage.getItem("pool_data_que"));
    ...
    function removeFromQue(o)
    {
        if(!queWork)
        {
            ...
            if(que != null)
            {
                ...
                if(que.length > 0)
                {
                    window.localStorage.setItem("pool_data_que", JSON.stringify(que));
                ...
                {
                    window.localStorage.removeItem("pool_data_que");
    ...
    function launchsaveOrder()
    {
        if(!pushed)
        {
            let savedList = JSON.parse(window.localStorage.getItem("pool_sys_saved_order"));
    ...
    function saveOrder()
    {
        ...
        if(order.items.length > 0)
        {
            ...
            if((listingName === "menu") || (listingName === "pool_sys_saved_order") || (listingName === ""))
            {
            ...
            else
            {
                ...
                window.localStorage.setItem("pool_sys_saved_order", JSON.stringify(savedList));
    ...
    function showMenu()
    {
        ...
        cover.innerHTML =
            ...
            "<i class='life ring outline icon'></i> pool Items</h3>" +
            "</div>" +
    ...
    function emptyTray()
    {
        $("#product-con").html("<tr>" +
            "<td colspan='5'>" +
            "<h3 class='sleak pad-2' style='color: silver;'>" +
            "<i class='factory circular blue-text icon'></i> pool Basket is empty</h3></td></tr>");
    ...
    function addOrderToQue(order)
    {
        ...
        que.push(order);
        window.localStorage.setItem('pool_que_store', JSON.stringify(que));
        window.localStorage.setItem('pool_data_que', JSON.stringify(que));

        window.localStorage.removeItem('pool_que_store');
