    ...
    function initializePOS()
    {
        ...
        cover.innerHTML =
            ...
            "<div class='ui inline active loader'></div> Initializing Kitchen POS" +
            "</h1><br/><br/>" +
    ...
    function initRoutine()
    {
        ...
        try
        {
            que = JSON.parse(window.localStorage.getItem("data_que"));
        }
        catch (e)
        {
            try
            {
                que = JSON.parse(window.localStorage.getItem('que_store'));
            }
            catch (e)
            {
    ...
    function runRoutine(o)
    {
        if(!queWork)
        {
            ...
            let que = JSON.parse(window.localStorage.getItem("data_que"));
    ...
    function removeFromQue(o)
    {
        if(!queWork)
        {
            ...
            if(que != null)
            {
                ...
                if(que.length > 0)
                {
                    window.localStorage.setItem("data_que", JSON.stringify(que));
                ...
                {
                    window.localStorage.removeItem("data_que");
        ...
        for(let i = 0; i < foodOrder.length; i++)
        {
            if((foodOrder[i] != null) && (!foodOrder[i].Fullfilled))
            {
                content +=
                    ...
                    "<br/><span>"+foodOrder[i].Foodpixel.length+" item(s)</span>" +
                    "</div> " +
        ...
        else
        {
            loadPageModal({size:"s", onLoaded:function(m){
                ...
                $("#modal_"+m.modal+"-inner").html(
                    ...
                    "<i class='blue utensils icon'></i> Food Order" +
                    "</h2>" +
                    "</div>" +
                    "</div>" +
    ...
    function openSingleOrder(e)
    {
        ...
        if(order !== null)
        {
            ...
            for(let i = 0; i < order.Foodpixel.length; i++)
            {
                list +=
                    "<div class='w3-row'>" +
                        "<div class='w3-col l4 m4 s4 pad-1'><img src='files/"+order.Foodpixel[i].Food.Images[0]+"' " +
                        "style='width: 50px; border-radius: 6px;'/></div>" +
                        "<div class='w3-col l5 m5 s5 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        order.Foodpixel[i].Food.Name+"</h6></div>" +
                        "<div class='w3-col l3 m3 s3 pad-1'><h6 class='sleak' style='font-weight: bold;'>"+
                        numFormat(Number(order.Foodpixel[i].Quantity))+"</h6></div>" +
                    "</div>";
            }
            content +=
    ...
    function buildWebOrder(e)
    {
        ...
        else
        {
            ...
            for(let i = 0; i < cc.length; i++)
            {
                order.items.push(cc[i].Food.Id+":"+cc[i].Quantity+":"+cc[i].Price+":"+cc[i].Tax);
                order.printItems.push({item:cc[i].Food.Name,qty:cc[i].Quantity,price:cc[i].Price});
            }
    ...
    function launchsaveOrder()
    {
        if(!pushed)
        {
            let savedList = JSON.parse(window.localStorage.getItem("sys_saved_order"));
    ...
    function saveOrder()
    {
        ...
        if(order.items.length > 0)
        {
            ...
            if((listingName === "menu") || (listingName === "sys_saved_order") || (listingName === ""))
            {
            ...
            else
            {
                ...
                window.localStorage.setItem("sys_saved_order", JSON.stringify(savedList));
        ...
        for(let i = 0; i < notifications.length; i++)
        {
            if((notifications[i] != null) && (!notifications[i].Fullfilled))
            {
                content +=
                    ...
                    "<br/><span>"+notifications[i].Foodpixel.length+" item(s)</span>" +
                    "</div> " +
                    "<div class='w3-col l3 m3 s3 align-r' style='line-height: 180%;'>" +
    ...
    function showMenu()
    {
        ...
        cover.innerHTML =
            ...
            "<i class='utensils icon'></i> Food Menu</h3>" +
            "</div>" +
    ...
    function addOrderToQue(order)
    {
        ...
        que.push(order);
        window.localStorage.setItem('que_store', JSON.stringify(que));
        window.localStorage.setItem('data_que', JSON.stringify(que));

        window.localStorage.removeItem('que_store');
